...
import com.xiaotiyun.school.manager.model.excel.SubjectImportModel;
import com.xiaotiyun.school.manager.model.excel.SubjectImportZhModel;
import lombok.Data;
...
@Data
public class SubjectImportListener extends AnalysisEventListener<SubjectImportZhModel> {
    private final List<SubjectImportZhModel> dataList = new ArrayList<>();

    // 存储期望的表头
    private static final Map<Integer, String> EXPECTED_HEADER = new HashMap<>();

    static {
        EXPECTED_HEADER.put(0, "科目編號（必填）");
        EXPECTED_HEADER.put(1, "科目名稱（必填）");
        EXPECTED_HEADER.put(2, "英文名称");
        EXPECTED_HEADER.put(3, "单位\n" +
                "（格式：数字，1-100）");
        EXPECTED_HEADER.put(4, "适用学部（必填）\n" +
                "支持输入：幼稚园、小学、中学\n" +
                "包含多个时：逗号“,”隔开");
    }
    ...
    @Override
    public void invoke(SubjectImportZhModel data, AnalysisContext context) {
    ...
    public List<SubjectImportZhModel> getDataList() {
        return dataList;
    }
}