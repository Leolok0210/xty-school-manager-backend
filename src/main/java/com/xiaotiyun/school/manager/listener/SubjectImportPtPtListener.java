...
import com.xiaotiyun.school.manager.model.excel.SubjectImportPtPtModel;
import lombok.Data;
...
@Data
public class SubjectImportPtPtListener extends AnalysisEventListener<SubjectImportPtPtModel> {
    private final List<SubjectImportPtPtModel> dataList = new ArrayList<>();

    // 存储期望的表头
    private static final Map<Integer, String> EXPECTED_HEADER = new HashMap<>();

    static {
        EXPECTED_HEADER.put(0, "Código da Disciplina (obrigatório)");
        EXPECTED_HEADER.put(1, "Nome da Disciplina (obrigatório)");
        EXPECTED_HEADER.put(2, "Nome em Inglês");
        EXPECTED_HEADER.put(3, "Unidade\n" +
                " (Formato: número, 1-100)");
        EXPECTED_HEADER.put(4, "Departamento Aplicável(obrigatório)\n" +
    ...
    @Override
    public void invoke(SubjectImportPtPtModel data, AnalysisContext context) {
    ...
    public List<SubjectImportPtPtModel> getDataList() {
        return dataList;
    }
}