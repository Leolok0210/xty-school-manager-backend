...
import com.xiaotiyun.school.manager.model.excel.UserImportEnUsModel;
import lombok.Data;
...
public class UserImportEnUsListener extends AnalysisEventListener<UserImportEnUsModel> {
    // 存储期望的表头
    private static final Map<Integer, String> EXPECTED_HEADER = new HashMap<>();
    private List<UserImportEnUsModel> dataList = new ArrayList<>();
    // 由于表头在第二行，我们需要一个标志来跳过第一行
    private boolean isHeaderRow = false;

    static {
        EXPECTED_HEADER.put(0, "Username (Required)");
        EXPECTED_HEADER.put(1, "Phone Number Region");
        EXPECTED_HEADER.put(2, "Mobile Number");
        EXPECTED_HEADER.put(3, "Full Name (Required)");
        EXPECTED_HEADER.put(4, "Department (Required)");
        EXPECTED_HEADER.put(5, "User Group (Required)");
        EXPECTED_HEADER.put(6, "User ID (Required)");
//        EXPECTED_HEADER.put(6, "Position (Required)");
        EXPECTED_HEADER.put(7, "Gender (Required)");
    }
    ...
    @Override
    public void invoke(UserImportEnUsModel importZhTwModel, AnalysisContext analysisContext) {
