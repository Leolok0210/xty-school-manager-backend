...
import com.xiaotiyun.school.manager.model.excel.UserImportZhTwModel;
import lombok.Data;
...
public class UserImportZhTwListener extends AnalysisEventListener<UserImportZhTwModel> {
    // 存储期望的表头
    private static final Map<Integer, String> EXPECTED_HEADER = new HashMap<>();
    private List<UserImportZhTwModel> dataList = new ArrayList<>();
    // 由于表头在第二行，我们需要一个标志来跳过第一行
    private boolean isHeaderRow = false;

    static {
        EXPECTED_HEADER.put(0, "用戶名（必填）");
        EXPECTED_HEADER.put(1, "手機號地區");
        EXPECTED_HEADER.put(2, "手機號碼");
        EXPECTED_HEADER.put(3, "用戶姓名（必填）");
        EXPECTED_HEADER.put(4, "所屬部門（必填）");
        EXPECTED_HEADER.put(5, "用戶組（必填）");
        EXPECTED_HEADER.put(6, "用戶編號（必填）");
//        EXPECTED_HEADER.put(6, "用戶職務（必选）");
        EXPECTED_HEADER.put(7, "性別（必填）");
    }
    ...
    @Override
    public void invoke(UserImportZhTwModel importZhTwModel, AnalysisContext analysisContext) {
