...
import com.xiaotiyun.school.manager.model.excel.TeacherAttendanceImportPtPtModel;
...
@Getter
public class TeacherAttendanceImportPtPtListener extends AnalysisEventListener<TeacherAttendanceImportPtPtModel> {
    private final List<TeacherAttendanceImportPtPtModel> dataList = new ArrayList<>();

    // 存储期望的表头
    private static final Map<Integer, String> EXPECTED_HEADER = new HashMap<>();

    static {
        EXPECTED_HEADER.put(0, "Número de Utilizador do Professor (obrigatório)");
        EXPECTED_HEADER.put(1, "Nome do Professor (obrigatório)");
        EXPECTED_HEADER.put(2, "Número do Cartão");
    ...
    @Override
    public void invoke(TeacherAttendanceImportPtPtModel importModel, AnalysisContext analysisContext) {
